...
Sprint 23 正在推进 `plan-027-stage-4-attractor-entry-plan`。`plan-026-v0-3-release-prep` 已关闭，阶段 3 功能队列收束为 v0.3.0 Verify Runner 正式发布；当前工作是把阶段 4 定为 Agent-First 吸引子入口层，并把 Agent-First Command Contract 写成 Stage 4 技术底座。
...
当前执行计划：`plan-027-stage-4-attractor-entry-plan`。
...
- 下一阶段焦点：阶段 4 Agent-First 吸引子入口层已从 `plan-027-stage-4-attractor-entry-plan` 启动。
...
- `plan-027-stage-4-attractor-entry-plan`（进行中）
- `plan-028-agent-first-command-contract`
- `plan-029-attractor-registry`
...
| 阶段 4：Agent-First 吸引子入口层 | `plan-001-sprint-1-foundation`, `plan-007-zero-dep-install`, `plan-027-stage-4-attractor-entry-plan` | active attractor 文档和模板；uvx/uv tool install 降低接入门槛；Agent-First Command Contract 正在纳入协议基线 | `abh/commands.py` 或等价 command contract、`abh attractor`、active attractor 校验、`abh init`、Agent Contract Export、Git hooks、`abh next --json`、onboarding check、quickstart/recipes、PyPI/uvx abh、完整 demo |
...
本节只列下一批仍可切分执行的计划。已关闭的 `plan-012-agent-protocol-foundation`、`plan-013-json-output-and-errors`、`plan-014-readonly-mcp-server`、`plan-015-controlled-mcp-write-tools`、阶段 3 的 `plan-016` 至 `plan-025` 和 release-prep 的 `plan-026-v0-3-release-prep` 已归入第 3 章历史执行线与第 6 章阶段映射；当前执行计划是 `plan-027-stage-4-attractor-entry-plan`。
