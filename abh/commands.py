...
COMMANDS: tuple[CommandContract, ...] = (
    ...
    CommandContract(
        ...
        read_only=True,
        confirmation="none",
        side_effects=["reads plans, health, doctor, roadmap"],
        description="Return a unified dashboard of repo state for agent navigation.",
        input_schema=input_schema({}),
        output_keys=["repo_state", "open_plans", "next_action", "health_posture", "active_alerts", "doctor_issues", "roadmap_queued"],
        failure_categories=["system"],
    ),
    CommandContract(
        id="onboarding.check",
        cli_command="onboarding check",
        mcp_tool="abh_onboarding_check",
        read_only=True,
        confirmation="none",
        side_effects=[],
