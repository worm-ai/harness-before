...
class CommandContractTests(WorkspaceCliTestCase):
    ...
    def test_stage7_docs_reconcile_ci_boundary_blocked_sharing_and_skill_slice(self) -> None:
        ...
        self.assertIn("all 67 plans closed; 4 roadmap items queued", task_board)
        self.assertIn("`stage7.multi-repo-sharing` → `plan-054-multi-repo-sharing`（closed, audit rejected, re-materialize needed）", task_board)
        self.assertIn("`adoption.abh-workflow-skill` -> `plan-055-abh-workflow-skill`（closed）", task_board)
        self.assertIn("Stage 7: `plan-053-ci-templates` is closed", codebase_map)
        self.assertIn("`plan-054-multi-repo-sharing` is closed (blocked/deferred", codebase_map)
        self.assertIn("`plan-055-abh-workflow-skill` is closed", codebase_map)
        self.assertIn("`plan-057-codex-repo-toggle` is closed", codebase_map)
